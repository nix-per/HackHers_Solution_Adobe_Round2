# THE SOLUTION WE HAVE PRESENTED IS PARTICULARLY FOR THE ISOLATED SHAPES PROBLEMS
# WE START BY DETECTING THE HAND-DRAWN SHAPES USING LOGICS OF GEOMETRY AND THEN REGULARIZING THEM
    ...
    for XYs in path_XYs:
        for XY in XYs:
            if is_line(XY):
                ...
                regularized_path_XYs.append([XY])  # If not recognized, keep the original
...
def regularize_rectangle(XY):
    ...
    # Ensuring there are approximately 4 vertices
    if len(vertices) < 4:
        return XY  # Returning the original if not enough vertices
    ...
    regularized_XY = np.array([
        ...
        [center[0] + width / 2, center[1] + height / 2],
        [center[0] - width / 2, center[1] + height / 2],
        [center[0] - width / 2, center[1] - height / 2]  # Closing the rectangle
    ])
...
def is_line(XY):
    ...
    # Consider it a line if more than a certain percentage of points are close to the fitted line
...
input_csv_path = 'problems\isolated.csv'           # KINDLY REPLACE THE PATH WITH ABSOLUTE PATH OF THE TEST FILES (CSV)
